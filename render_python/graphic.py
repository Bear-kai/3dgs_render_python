...
def getProjectionMatrix(znear, zfar, fovX, fovY):
    ...
    P[3, 2] = z_sign                            # 在near和far为非负标量的情况下，该值为1，说明推导时的cam为左手系！
    P[2, 2] = z_sign * zfar / (zfar - znear)      
    P[2, 3] = -(zfar * znear) / (zfar - znear)  # near --> 0, far --> 1，通过映射后的取值大小，可看出NDC空间为左手系！
    return P
...
def getProjectionMatrix_games101(znear, zfar, fovX, fovY):  # 自增
    ...
    P[3, 2] = 1.0                                 # 在near和far为非负标量的情况下，该值为1，说明推导时的cam为左手系！
    P[2, 2] =  (znear + zfar) / (znear - zfar)
    P[2, 3] = -2.0*znear*zfar / (znear - zfar)    
                                                  # 这里 near --> 1, far --> -1，有NDC为右手系
    return P
...
def getProjectionMatrix_opengl(znear, zfar, fovX, fovY):    # 自增
    ...
    P[0, 0] = 2.0 * znear / (right - left)        # 2n/W
    P[1, 1] = 2.0 * znear / (top - bottom)        # 2n/H
    P[3, 2] = -1.0                                # 在near和far为非负标量的情况下，该值为-1，说明推导时的cam为右手系！
    P[2, 2] = (znear + zfar) / (znear - zfar)
    P[2, 3] = 2.0*znear*zfar / (znear - zfar)
                                                  # 这里 near --> -1, far --> 1，有NDC为左手系！
    return P
